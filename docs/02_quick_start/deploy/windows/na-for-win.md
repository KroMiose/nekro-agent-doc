---
title: Nekro Agent Windows安装器（基于WSL2）
description: 使用安装器的一键安装教程。
---

# NA安装器使用流程

本文档将以截图的形式，展示Nekro Agent Windows安装器(下称安装器)的使用流程。
...
2. **运行环境检测，部署服务**
    ...
    - 点击创建后，安装器会自行下载创建wsl环境，耐心等待即可
    - 等到wsl环境初始化完成后，将进入正式部署环节，首先安装器会让您选择部署版本，若您第一次接触`Nekro Agent`或AI机器人，推荐您选择完整版，如下图所示
    ![选择版本](/assets/windows/manage3.png)
    - 选择版本后，安装器会让您选择端口号，一般**默认**即可，若出现端口冲突，则可进行更改，上方的路径可在部署完成后复制进文件资源安装器，用于访问Nekro Agent数据目录，如下图所示
    ![详细配置](/assets/windows/manage4.png)
    - 配置完成后，安装器会自动拉取所需镜像，由于镜像文件较大，下载所需时间稍长，耐心等待即可，如下图所示
    ![下载](/assets/windows/manage5.png)
    - 下载完成后，安装器会自动进入启动流程，过程中会弹出用于展示相关凭据的弹窗，请妥善保存，如下图所示
    ![凭据](/assets/windows/manage6.png)
    - 服务成功启动后，弹窗可被关闭，安装器同时会跳转至内置浏览器，您可根据使用需求切换相应服务（如有），同时，您可以点击填充凭据按钮自动填充na/napcat的登录凭据，免去记忆凭据的过程，如下图所示
    ![内置浏览器](/assets/windows/manage7.png)
    - 到这里，NekroAgent的部署已经完成。

## ⚙️ 部署后配置

- **标准部署（对应安装器完整版）**: 前往[协议端配置](/docs/02_quick_start/config/protocol.html#napcat-集成部署-推荐)文档，根据文档说明完成配置。
- **核心部署（对应安装器精简版）**: 前往[协议端配置](/docs/02_quick_start/config/protocol)文档，参考说明继续配置。
