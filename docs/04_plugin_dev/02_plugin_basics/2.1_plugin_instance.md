...
plugin = NekroPlugin(
    ...
    allow_sleep=True,  # 允许休眠（节省上下文），需同时填写 sleep_brief
    sleep_brief="做很多酷炫的事情，当用户明确需要时激活。",
)
```
...
- `i18n_name` (Dict[str, str], 可选): 插件名称的国际化映射，键为语言代码（如 `zh_CN`、`en_US`），值为对应语言的名称。设置后会覆盖 `name` 在对应语言下的展示。通常配合 `i18n.i18n_text()` 使用。
- `i18n_description` (Dict[str, str], 可选): 插件描述的国际化映射，格式与 `i18n_name` 相同。
- `allow_sleep` (bool | None, 可选, 默认为 `None`): 控制插件是否参与[激活调度](./2.7_plugin_activation)（休眠机制）。
  - `None`（默认）：不参与调度，插件始终常驻展示（保守策略，兼容老插件预留，不建议使用该值）。
  - `False`：强制禁止休眠（适合基础能力插件或短上下文工具插件，如消息发送）。
  - `True`：允许休眠，但必须同时提供 `sleep_brief`，否则回退为不可休眠。
- `sleep_brief` (str, 可选, 默认为 `""`): 插件处于休眠状态时展示给 AI 的能力摘要。**设置 `allow_sleep=True` 时必须填写**，AI 将通过此摘要判断是否需要唤醒插件。

`NekroPlugin` 实例 (`plugin`) 将作为后续所有装饰器的调用对象，用于挂载沙盒方法、配置、生命周期回调等。

### 插件日志实例 (`plugin.logger`)

每个插件实例都自带一个 **`plugin.logger`**，用于在插件内打日志。推荐在插件代码中统一使用 `plugin.logger` 而非全局 `core.logger`：

*   **按插件过滤**：日志会带上 `plugin_key`（如 `Author.module_name`），管理员可在 WebUI「日志」页按插件筛选，快速定位该插件的记录。
*   **用法与标准 logger 一致**：支持 `debug`、`info`、`warning`、`error`、`exception` 等，例如：
    *   `plugin.logger.info("任务已创建")`
    *   `plugin.logger.exception("处理失败")`  # 会附带堆栈

```python
# 推荐：在挂载的方法、初始化、Webhook 等逻辑中使用
plugin.logger.info(f"插件 '{plugin.name}' 正在初始化...")
plugin.logger.warning("配置项缺失，使用默认值")
plugin.logger.error(f"请求失败: {e}")
```
...
async def initialize_plugin():
    ...
    try:
        ...
        if collection_name not in collection_names:
            plugin.logger.info(f"正在创建向量数据库集合: {collection_name}")
            ...
            plugin.logger.success(f"集合 {collection_name} 创建成功")
        else:
            plugin.logger.info(f"集合 {collection_name} 已存在")
    except Exception as e:
        plugin.logger.error(f"插件 '{plugin.name}' 初始化向量数据库失败: {e}")
    plugin.logger.success(f"插件 '{plugin.name}' 初始化完成。")
```
...
async def handle_channel_reset(_ctx: AgentCtx):
    plugin.logger.info(f"插件 '{plugin.name}' 收到会话 {_ctx.from_chat_key} 重置事件。")
    # 示例：清除该会话的插件特定缓存
    # await plugin.store.delete(chat_key=ctx.from_chat_key, store_key="session_cache")
    plugin.logger.info(f"会话 {_ctx.from_chat_key} 的插件特定数据已清理。")
```
