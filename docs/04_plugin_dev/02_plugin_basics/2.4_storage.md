...
# 存储会话特定数据
await plugin.store.set(chat_key=ctx.from_chat_key, store_key="last_command", value="/weather London")

# 存储用户特定偏好
user_prefs = {"theme": "dark", "notifications": True}
await plugin.store.set(user_key=ctx.from_user_id, store_key="preferences", value=json.dumps(user_prefs))
...
```python
# 获取会话特定数据
last_command = await plugin.store.get(chat_key=ctx.from_chat_key, store_key="last_command")

# 获取用户偏好，如果不存在则使用默认值
prefs_str = await plugin.store.get(user_key=ctx.from_user_id, store_key="preferences")
user_preferences = {} 
if prefs_str:
...
```python
# 删除会话的特定缓存
await plugin.store.delete(chat_key=ctx.from_chat_key, store_key="session_cache_data")

# 删除用户的某个设置
await plugin.store.delete(user_key=ctx.from_user_id, store_key="old_setting")
```
...
```python
value = await plugin.store.get(chat_key=ctx.from_chat_key, store_key="my_key")
...
async def add_note(ctx: AgentCtx, note_id: str, title: str, content: str, tags_str: str = "") -> str:
    # 1. 获取现有笔记数据
    user_notes_json = await plugin.store.get(user_key=ctx.from_user_id, store_key="all_notes")
...
async def get_note_content(ctx: AgentCtx, note_id: str) -> Optional[str]:
    user_notes_json = await plugin.store.get(user_key=ctx.from_user_id, store_key="all_notes")
