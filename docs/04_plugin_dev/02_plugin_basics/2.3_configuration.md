...
class MyPluginConfig(ConfigBase):
    ...
    max_items: int = Field(
        ...
        ge=1,  # Pydantic 验证：大于等于 1
        le=100 # Pydantic 验证：小于等于 100
    )
    ...
    class Config:
        # Pydantic 配置，例如
        validate_assignment = True # 当字段被重新赋值时也进行验证
```
...
| 浮点数              | `float`                  | 数字输入框 (带小数)     |
| 布尔值              | `bool`                   | 开关 (Toggle)           |
| URL                 | `HttpUrl`                | 文本输入框 (带 URL 验证) |
| 枚举 (固定选项)     | `Literal["a", "b"]`    | 下拉选择框              |
| 字符串列表          | `List[str]`              | 多行文本输入/标签输入器  |
| 字典                | `Dict[str, Any]`         | JSON 编辑器 (可能)      |
| 自定义 Pydantic 模型 | `MySubConfigModel`       | 嵌套表单 (可能)         |
...
async def my_action(ctx: AgentCtx, some_input: str) -> str:
    ...
    service_url = str(plugin.config.service_url) # HttpUrl 类型需要转换为 str 使用
...
由于配置类是 Pydantic 模型，你可以享受到类型提示、自动完成和运行时类型检查的好处。

## 配置验证

Pydantic 模型本身就带有强大的数据验证功能。除了在 `Field` 中直接定义的约束外，你还可以通过定义 `@validator` 或 `@field_validator` (Pydantic V2) 方法来实现更复杂的自定义验证逻辑。
...
class AdvancedConfig(ConfigBase):
    custom_port: int = Field(default=8080, title="自定义端口")
    another_value: str
    ...
    def validate_port_range(cls, value):
        if not (1024 <= value <= 65535):
            raise ValueError("端口号必须在 1024 到 65535 之间。")
        return value

    # 可以定义检查多个字段的 validator
    # from pydantic import model_validator
    # @model_validator(mode='after')
    # def check_ взаимосвязанные_fields(self) -> 'AdvancedConfig':
    #     if self.field_a == "specific_value" and self.field_b <= 10:
    #         raise ValueError("当字段A为特定值时，字段B必须大于10")
    #     return self
```

如果用户在 WebUI 中输入的配置值未能通过验证，系统会提示错误，阻止无效配置的保存。
...
4.  **最小权限原则**：如果插件需要 API 密钥，请在描述中建议用户生成具有最小必需权限的密钥。
5.  **优雅降级**：当某些配置项缺失或无效时，插件应尝试优雅地处理（例如，禁用依赖该配置的功能，并给出明确提示），而不是直接崩溃。
6.  **配置热更新**：Nekro Agent 通常支持配置的即时生效，无需重启插件或 Agent。在插件代码中访问 `plugin.config` 时，获取到的通常就是最新的配置值。
